<template>
  <div class="banner-container">
    <swiper :style="{width: '100vw', height: '100%'}" 
        ...
        <div class="image-container" 
          ...
          <image :src="item.picture" 
			...
			<view class="image-detail">
				<text class="image-detail-name">{{item.title}}</text>
				<text class="image-detail-num">单词总数：{{item.word_num}}</text>
			</view>
        </div>
      </swiper-item>
    </swiper>
...
export default {
  ...
  methods: {
    swiperChange (e) {
	  ...
	  this.$emit('select_change',this.curIndex)
    },
    animationfinish (e) {
